...
results_path = 'C:\\Users\KLA\Dropbox\\MHC OPS Absent Narratives Approach\\Evaluation (All) 2016-2017\\Evaluation Methods\\Surveys\\Post Offering Surveys\\Secrets to Motivating Students\\sms_fa16_survey_export.csv'
results = pd.read_csv(results_path, encoding = "ISO-8859-1")

###create object for file name

results_name = ntpath.basename(results_path).rstrip('_export.csv')
...
codebook = pd.read_csv('C:\\Users\\KLA\\Dropbox\\MHC OPS Absent Narratives Approach\\Evaluation (All) 2016-2017\\Evaluation Methods\\Surveys\\survey_codebook_fa16.csv', encoding = "ISO-8859-1")
...
results['semester']= results_name_simple.split('_', 1)[1]
#comment out next line for rc
results['offering']= results_name_simple.split('_', 1)[0]
...
results.to_csv('C:\\Users\\KLA\\Dropbox\\MHC OPS Absent Narratives Approach\\Evaluation (All) 2016-2017\\Evaluation Methods\\Surveys\\cleaned_survey_results\\' + results_name + '_cleaned.csv')
...
for item in empathy_list:
	item = ([results[item +'_before'].astype(float).mean(), results[item + '_now'].astype(float).mean(), len(results[item +'_before']), len(results[item + '_now'])])
